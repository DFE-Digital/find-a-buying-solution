...
namespace :contentful do
  desc "Upload new or updated translations from en.yml to Contentful and republish them"
  task en_to_contentful_translations_create_or_update: :environment do
    ...
    begin
      ...
      # puts "Fetched #{contentful_translations.size} existing translations from Contentful."
      # puts "Loaded #{flat_translations.size} local translations from en.yml."
      ...
      (new_translations.merge(updated_translations)).each do |key, value|
        ...
        begin
          create_or_update_contentful_entry(key, value, entry, space_id, token)
